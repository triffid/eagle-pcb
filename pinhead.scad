		...
		for (r = [0:rows - 1])
			for (p = [0:pins - 1]) {
				translate([(p - (pins - 1) / 2) * pitch, -r * pitch - 3.81, 0]) {
					hull() {
						translate([0, 0, -2.2]) rotate([0, 0, 180 / 4]) cylinder(d=0.6 / cos(180 / 4), h=2.2 + 0.28 + ee + (r * pitch), $fn=4);
						translate([0, 0, -2.5]) rotate([0, 0, 180 / 4]) cylinder(d=0.3 / cos(180 / 4), h=2.5 + 0.28 + ee + (r * pitch), $fn=4);
					}
